    ...
    actions: {
        ...
        async save({ state, commit, dispatch }) {
            ...
            sizeGuideData.append(
                "shoulder",
                Math.round(state.editedItem.shoulder)
            );
            sizeGuideData.append("bust", Math.round(state.editedItem.bust));
            sizeGuideData.append("wist", Math.round(state.editedItem.wist));
            ...
            if (state.editedIndex === -1) {
                ...
                const Data = await axios
                    .post(
                        `/sizeGuide/update/${state.editedIndex}`,
                        sizeGuideData
                    )
                    .catch((error) => {
                        commit("fillDialogErrors", error.response.data.errors);
